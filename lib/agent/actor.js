...
var Actor = function (conf, aid) {
    ...
    this.id = aid;
    this.config = conf;
    this.script = conf.script || fs.readFileSync(process.cwd() + envConfig.script, 'utf8');
    var self = this;
...
pro.run = function () {
    var self = this;
    try {
        // xykong require file so we can print line number in debugging.
        if (self.config != null && self.config.scriptFile.length != 0) {
            actor = this;
            require(self.config.scriptFile);
        }
        else {
            var initSandbox = {
                console: console,
